...
void TestInspect(int a, int b) {
  ...
  // CHECK: InspectStmt 0x{{[^ ]*}} <line:[[@LINE-3]]:3, line:[[@LINE-1]]:3> has_implicit_result_type
  // CHECK: IdentifierPatternStmt 0x{{[^ ]*}} <line:[[@LINE-3]]:5, col:9>
  // CHECK-NEXT: UnaryOperator 0x{{[^ ]*}} <col:8, col:9> 'int':'int' postfix '++'
  // CHECK-NEXT: DeclRefExpr 0x{{[^ ]*}} <col:8> 'int':'int' lvalue Var 0x{{[^ ]*}} 'y' 'int &'
  // CHECK-NEXT: DeclStmt 0x{{[^ ]*}} <col:6>
